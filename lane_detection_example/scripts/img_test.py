...
from utils import BEVTransform, CURVEFit, draw_lane_img, purePursuit, PID_longitudinal
...
class LINEDetector:
	...
	lower_lane = np.array([ 0,   0, 250]) # 0,   0, 250  0  0 220
	upper_lane = np.array([40, 255, 255]) # 37, 255, 255 40 10 255

	img_wlane = cv2.inRange(img_hsv, lower_lane, upper_lane)
	
	self.outimg = np.concatenate([img2,img], axis=1)
	return img_wlane
